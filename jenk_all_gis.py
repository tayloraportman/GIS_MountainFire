...
from qgis.core import (
    QgsProject, QgsPrintLayout, QgsTextFormat, QgsLayoutItemMap, QgsLayoutExporter,
    ...
    QgsMapLayerLegendUtils, QgsRectangle,QgsRasterLayer, QgsCoordinateReferenceSystem
)
from qgis.PyQt.QtGui import (QColor, QFont)
from qgis.PyQt.QtCore import QRectF
import jenkspy

# Set CRS for the QGIS project
crs = QgsCoordinateReferenceSystem('EPSG:4326')  # Replace with your desired EPSG code
...
def prepare_layout(layer, field_name):
    ...
    title.setText(f"Mountain Fire {field_name}")
    title_font = QFont("Arial", 16, QFont.Bold)
    title.setFont(title_font)

    # Set text color using QgsTextFormat
    text_format = QgsTextFormat()
    text_format.setFont(title_font)
    text_format.setColor(QColor(255, 255, 255))  # Set text color to white
...
if active_layer is None:
    ...
    if active_layer.type() == QgsMapLayer.VectorLayer:
        print("Active layer is a vector layer.")
        exclude_columns = ['GPSLon', 'GPSLat', 'GPSAlt']
        # Example vector layer processing
        for field in active_layer.fields():
            if field.name().lower() not in exclude_columns:
                apply_graduated_symbology_and_export(active_layer, field.name())
    elif active_layer.type() == QgsMapLayer.RasterLayer:
        print("Active layer is a raster layer.")
        # Insert raster-specific logic here if needed